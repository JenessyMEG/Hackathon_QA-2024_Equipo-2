...
# Prueba el filtrado por título del producto
    def test_filter_by_title(self):
        response = self.get_response(
            {"title": "Classic Red Jogger Sweatpants"})  # Se cambia "Generic" por un título valido
        products = response.json()
        print("Filter by title response:", products)
        self.assertTrue(any("Classic Red Jogger Sweatpants" in product.get('title', '') for product in products),
                        "No products with title 'Classic Red Jogger Sweatpants' found")
